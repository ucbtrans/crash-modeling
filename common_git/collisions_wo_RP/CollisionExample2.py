...
# Additional intermediate control points for Car 1 and Car 2 - We will use cubic spliing here to generate our path 
control_points_p1 = np.array([[A['x'], A['y']], [-26.8,24.4],[-26.4,6.4],[collisionvertex['x'], collisionvertex['y']+1]])  # Car 1
control_points_p2 = np.array([[B['x'], B['y']], [-37,-4.8],[-27.6,-3.4] ,[collisionvertex['x'], collisionvertex['y']]])  # Car 2
...
# Now construct the KSState objects using the interpolated points - We will also calculate the orientation of the vehicle at each point
...
# scenario_file = "USA.xml"

# CommonRoadFileWriter(scenario,planning_problem_set).write_scenario_to_file(scenario_file)
